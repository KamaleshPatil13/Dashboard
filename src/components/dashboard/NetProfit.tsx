...
const NetProfit = ({ netProfit, goalPercentage, increasePercentage }: NetProfitProps) => {
  return (
        ...
        <div className="relative grid">
          <div className="w-24 h-24 rounded-full bg-gray-700">
            <div className="absolute top-0 left-0 w-full h-full rounded-full bg-gradient-to-r from-blue-500 to-purple-500">
              <div className="absolute -top-8 right-0 bottom-0 left-0 flex justify-center items-center text-white font-bold text-[20px]">
                {goalPercentage}%
              </div>
            </div>
          </div>
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-2 text-sm text-gray-400 text-center text-[10px]">
            Goal Completed
          </div>
        </div>
      </div>
